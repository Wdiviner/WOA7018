...
def detect(opt, save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else '_{}'.format(frame))  # img.txt
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if round(conf.item(), 2) > 0.7 :
                            pub.publish('a dangerous good is detected,' + names[int(cls)])
...
my_thread = threading.Thread(target=detect, args=[opt])

def message_callback(msg):
    rospy.loginfo("Received message: %s", msg.data)

    if str(msg.data)=='start' or str(msg.data)=='begin':
